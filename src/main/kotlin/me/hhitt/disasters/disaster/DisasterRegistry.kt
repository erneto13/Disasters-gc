...
/**
 * DisasterRegistry is a singleton object that manages the active disasters in the game. It allows
 * adding, removing, and pulsing disasters for each arena. Now includes priority-based selection and
 * exclusion rules.
 */
object DisasterRegistry {
    ...
    fun addRandomDisaster(arena: Arena) {
        ...
        val availableByTime =
                DisasterConfig.getAvailableDisastersByTime(disasterClasses, gameTimePercent)

        val availableClasses =
                availableByTime.filter { cls -> currentDisasters.none { it::class == cls } }
